...
1. X86有几个特权级？
   ...
   x86一共有4个特权级(用2bit来表示)，但一般情况下只使用0和3两个特权级。


2. 不同特权级有什么区别？
   * 使用的指令不同。特权指令只能在内核态使用。
   * 访问的内存空间和数据范围不同。
   * 某些指令在不同特权级下运行的效果也不同。


3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。
   * 中断响应和系统调用时，可能存在访问高特权级的情况。此时CPL$$\le$$gate DPL, CPL $$\ge$$ 目的段DPL。
   * 在访问段时，$$MAX(CPL, RPL) \le  DPL$$
...
1. 一条指令在执行时会有哪些可能的特权级判断？
   * 访问数据时需要检查当前特权级和数据段的特权级。
   * 访问其他段时，需要检查当前段和目的段的特权级。
   * 发生中断或异常时，需要检查当前特权级和门特权级。
2. 在什么情况下会出现特权级切换？
   * 出现中断或者异常
   * 用户程序进行系统调用
3. int指令在ring0和ring3的执行行为有什么不同？
   * 如果是在用户态执行，需要将用户栈转换成内核栈，所以多压入ss和esp的内容。


4. 如何利用int和iret指令完成不同特权级的切换？
   * 在中断处理程序中，将栈内保存的寄存器信息进行修改。之后调用iret时，就会转换成相应的特权级。


5. TSS和Task Register的作用是什么？
   * TSS：存储着关于Task的信息，可以用来切换进程。如寄存器状态、I/O口许可、转换成特权级时的堆栈信息。
   * Task Register：存储指向TSS描述符的段选择子。
...
1. 一条指令执行时最多会出现多少次地址转换？
   ...
   两次：从逻辑地址转换成线性地址，再从线性地址转换成物理地址。

2. 描述X86-32的MMU地址转换过程；

   先通过段机制转换成线性地址，再通过页机制转换成物理地址。

### 7.4 了解UCORE建立段/页表

1. 分析MMU的使能过程，尽可能详细地分析在执行进入保护械的代码“movl %eax, %cr0 ; ljmp $CODE_SEL, $0x0”时，CPU的状态和寄存器内容的变化。

   CPU开启段机制。在访问逻辑地址时通过段机制转换成线性地址。

   cr0寄存器中的段标志位开启，通过段寄存器保存的选择子在GDT中找到基址。

2. 分析页表的建立过程；

   在加载内核时已经预先建立好0～4M和0xC0000000~0xC0400000映射到0～4M的页表。在开启页机制后，将逻辑地址跳到0xC0000000开始的地址，再清除0～4M的映射，重新建立页表。
...
2. (easy) Linux和Windows分别采用哪种映射机制（线性映射or自映射）？它们的选择背后有什么原因吗？
   * windows采用了自映射机制。主要是因为windows每个进程独享全部虚拟地址空间。
   * Linux使用了线性映射。在Linux中，每个进程对于内核态的地址空间是共享的，所以会使得虚拟空间不足。故Linux设想不再用虚拟地址映射页表，而是使用线性映射。
