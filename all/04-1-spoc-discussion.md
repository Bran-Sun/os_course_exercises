...
2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？
   ...
   * 每种存储单元都处于繁忙状态，且未到达瓶颈。

3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？

   遇到过。将运行时的部分数据迁移到外存中，或是关闭其他占用内存的进程。

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？
   * 根据程序的逻辑结构，将程序划分为不同的模块，将不会同时运行的模块共享一块内存空间。
   * 程序员需要划分程序的结构，并安排哪些模块可以共享同一块内存空间。
2. 什么是交换技术？覆盖与交换有什么不同？
   * 将暂时不运行的程序放到外存中。
   * 覆盖主要针对一个进程，让其占用更小的内存空间。交换涉及多个进程，是将等待的进程放到外存中，从而给将运行的程序提供空间。
3. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？
   * 覆盖技已经不太使用，但交换技术的思想还是很普遍的。现在的虚拟存储系统中，就用到了这些思想。
   * 覆盖技术曾在DOS系统的Turbo Pascal中使用。
4. 如何分析内核模块间的依赖关系？
   * 使用depmod等命令。
5. 如何获取内核模块间的函数调用列表？
   * 静态分析代码，或者使用一些工具来生成。
...
1. 什么是时间局部性、空间局部性和分支局部性？
   ...
   * 时间局部性：在很短的时间内，可能访问相同位置的内存地址。
   * 空间局部性：可能在较短时间内访问的内存地址在一个较小区域内。
   * 分支局部性：一条跳转指令的两次执行，很可能跳转到相同的内存地址。
...
1. 什么是虚拟存储？它与覆盖和交换的区别是什么？它有什么好处和挑战？
   * 思路：将不常用的内存块转移到外存。
   * 装载程序时只将当前需要的部分加载到内存中。在运行时如果发生缺页，就将内存中暂时不用的部分转移到外存中。
   * 好处：可以提供给用户更大的逻辑地址空间。分配内存更加灵活。
   * 挑战：内存管理更加复杂。
2. 虚拟存储需要什么样的支持技术？
   * 硬件：地址转换机制。
   * 操作系统：管理内存和外存的换入换出。
